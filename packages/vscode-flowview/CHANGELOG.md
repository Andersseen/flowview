...
- Make the Astro snippet emit the complete
  `<template flowview is:raw context={...}>` editor-compatible wrapper.
- Add executable grammar checks.

## 0.1.0

- Initial release.
- Syntax highlighting for flowview templates (`.flow`).
...
- Grammar injection for inline flowview inside Astro `<template flowview>` blocks.
- Legacy grammar injection for inline flowview inside Astro `<flowview is:raw>` blocks.
- Snippets for flowview and Astro usage.
